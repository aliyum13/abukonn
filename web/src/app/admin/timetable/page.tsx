...
const emptyClass = (): Omit<TimetableClass, 'id'> => ({
  ...
  course_code: '', course_title: '', venue: '', lecturer: '',
});
              ...
              <div className="overflow-x-auto rounded-xl border border-border">
                <table className="w-full text-left text-[13px]">
                  <thead className="border-b border-border bg-surface-muted">
                    <tr>
                      {['Day','Time','Code','Title','Venue','Lecturer',''].map(h => (
            ...
            <div>
              ...
              <div className="overflow-x-auto rounded-xl border border-border">
                <table className="w-full text-left text-caption">
                  <thead className="border-b border-border bg-surface-muted">
                    <tr>{['day','start_time','end_time','course_code','course_title','venue','lecturer'].map(h => <th key={h} className="px-3 py-2 font-semibold text-ink capitalize">{h.replace('_',' ')}</th>)}</tr>
                  </thead>
                  <tbody className="divide-y divide-border">
                    {preview.map((row, i) => (
                      <tr key={i}>{['day','start_time','end_time','course_code','course_title','venue','lecturer'].map(h => <td key={h} className="px-3 py-2 text-ink-secondary">{row[h] || '—'}</td>)}</tr>
                    ))}
                  </tbody>
                </table>
              </div>
