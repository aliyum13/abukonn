...
// Inner component that reads the URL query params. Must live inside a
// <Suspense> boundary (see default export) -- Next.js requires useSearchParams
// to be suspense-wrapped so the page can be statically generated.
function ProCallbackInner() {
  const { token } = useAuth();
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-white px-4 dark:bg-[#0a0a0a]">
      <Suspense fallback={
        <div className="w-full max-w-sm text-center">
          <div className="mx-auto mb-4 h-10 w-10 animate-spin rounded-full border-4 border-brand-200 border-t-brand-600" />
          <p className="text-[15px] font-medium text-ink">Loading…</p>
        </div>
      }>
        <ProCallbackInner />
      </Suspense>
